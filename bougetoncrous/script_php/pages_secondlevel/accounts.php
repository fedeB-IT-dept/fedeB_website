...
function mail_confirmation_subscription($mail,$hash_mail,$cconf)
{
	$nexp=NOM_ECOLE. "";
	$email=MAIL_CONTACT;
	$subject="Confirmation de votre inscription à ".NOM_ECOLE." ";
...
function mail_change_password($mail, $new_pass)
{
	$nexp=NOM_ECOLE."";
	$email=MAIL_CONTACT;
	$subject="Changement mot de passe ".NOM_ECOLE."";
	$header = "From: ". $nexp . " <" . $email . ">\r\n";
							...
							if (is_ecole_mail($mail))
							{
								...
								else
								{
									...
									else
									{
										...
										if (@mysql_query("INSERT INTO `user` (`user_id`,`hash_mail`,`hash_pass`,`hash_conf`,`inscription_date`,`privileges`,`is_valid`) VALUES (NULL, '$hash_mail', '$hash_pass', '$hcconf', CURRENT_TIMESTAMP, 3, 1)"))
										{
											mail_confirmation_subscription($mail,$hash_mail,$cconf);
											echo('<div class="success">Inscription faite avec succ&egrave;s.</div>');
											$affich_form=false;
										}
										else
										{
...
<br />
		<h1>J\'ai d&eacute;j&agrave; un compte</h1>
			  		     <form method="post" action="?action=login">
									<table class="tab_form">
												<tr>
															  <td>
															        Nom d\'utilisateur '.((LIMIT_MAIL)?'('.PREGMATCH_MAIL_HUMAN_READABLE.')':'').' :
																    		   							         </td>
																										      <td>
																										            <input type="text" name="login" value="'.htmlentities($login_clair).'" />
																											    	   	       		    					    </td>
	...
	if (isset($_GET["mail"]) && is_string($_GET["mail"]) && isset($_GET["cconf"]) && is_string($_GET["cconf"]))
	{
		...
		else
		{
			...
			while ($row=mysql_fetch_assoc($result))
			{
				...
				{
					echo('<div class="success">Inscription valid&eacute;e, vous pouvez d&eacute;sormais vous <a href="?action=create_account">connecter</a></div>');
				}
				else
				{
		...
		if ($_POST['form_name']=="mdp_oubli" && $forgotten_passw)
		{
			if (isset($_POST["mail"]) && !empty($_POST["mail"])) // On ne vérifie pas le type "x.x@eleves.ecole.fr" pour permettre la gestion de comptes spéciaux (administration, ...)
			{
				...
				if ($res_q)
				{
					...
					if ($nb_enr<1)
					{
						echo('<div class="warning">Le nom d\'utilisateur ne semble pas inscrit</div>');
					}
					else
					{
						$affich_form=false;
		...
		elseif ($_POST['form_name']=="mdp_change" && !$forgotten_passw && $is_logged)
		{
			...
			if ($treat)
			{
				if(isset($_POST["mail"]) && !empty($_POST["mail"]))
				{
					...
					{
						$treat=false;
						echo('<div class="warning">Le nom d\'utilisateur ne correspond pas &agrave; celui avec lequel vous vous &ecirc;tes logg&eacute;</div>');
					}
				}
				else
				{
		...
		if ($forgotten_passw)
		{
			...
			<form method="post" action="?action=lost_ids">
				<table class="tab_form">
					<tr>
						<td>
							Nom d\'utilisateur inscrit '.((LIMIT_MAIL)?'('.PREGMATCH_MAIL_HUMAN_READABLE.')':'').' :
						</td>
						<td>
		...
		elseif ($is_logged)
		{
			...
			<form method="post" action="?action=change_pass">
				<table class="tab_form">
					<tr>
						<td>
							Nom d\'utilisateur (confirmation de l\'identit&eacute;) :
						</td>
						<td>
							<input type="text" name="mail" value="'.htmlentities($default_value_mail).'" />
						</td>
		...
		if (isset($_POST['form_name']) && $_POST['form_name']=="delete_account")
		{
			...
			if ($treat) // Vérifications sur les arguments
			{
				if(isset($_POST["mail"]) && !empty($_POST["mail"]))
				{
					...
					{
						$treat=false;
						$retour.='<div class="warning">Le nom d\'utilisateur ne correspond pas &agrave; celui avec lequel vous vous &ecirc;tes logg&eacute;</div>';
					}
				}
				else
				{
