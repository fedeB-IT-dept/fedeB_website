...
function add_document()
{
	...
	if (user_privilege_level()>3) // Administrateurs - modérateurs
	{
		...
		if(isset($_POST['form_name']) && $_POST['form_name']=="new_document")
		{
			...
			{
				$name_newfile=sha1(uniqid('f')).'.swf'; // Génération d'un nom qui soit unique (statistiquement) et non devinable
				//$date = date("d-m-Y-H-i-s");
				//$name_newfile='.escapeshellarg($_FILES['uploaded_file']['tmp_name']).' . $date;
				$commande='".\lect_flash\pdf2swf" '.escapeshellarg($_FILES['uploaded_file']['tmp_name']).' ".\rep_documents\\'.$name_newfile.'" -T 9 -f';
				@exec($commande,$vinu,$res_comm);
				if ($res_comm!=0) // Conversion du pdf uploadé en flash stocké
				{
					$retour.='<div class="warning">Impossible de r&eacute;cup&eacute;rer ou convertir le fichier " . echo $res_comm . " " . echo $name_newfile</div>';
				}
				else
				{
