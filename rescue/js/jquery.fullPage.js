...
(function($) {
	$.fn.fullpage = function(options) {
		// Ensemble des paramètres proposés avec des valeurs par défaut
		options = $.extend({
		...
		// Définition du délai prenant place avant le scroll jusqu'à la section suivante
		// ATTENTION! Il n'est pas recommandé de le passer en-dessous de 400 pour un comportement correct sur les laptops et 
		// appareils Apple (laptops, souris...)
		var scrollDelay = 700;
		...
		// "Flag" empêchant un glissement trop rapide sur les slides en paysage
		...
		// Si le CSS3 n'est pas supporté, on utilise des animations jQuery
		if(options.css3){
		...
		// Céation des puces de navigation
		...
		$('.section').each(function(index){
			...
			if (numSlides > 0) {
				...
				$(this).find('.controlArrow.next').css('border-color', 'transparent transparent transparent transparent');
				$(this).find('.controlArrow.prev').css('border-color', 'transparent transparent transparent transparent');
			...
			// Centrage vertical pour la navigation + first bullet active
			if(options.navigation){
		...
		/* Détection des actions tactiles
		...
		*/
		...
		/**
		 * Détection des actions de la roulette de souris
		 * 
		...
		function scrollPage(element, callback) {
			...
			if(options.css3 && options.autoScrolling){
				...
				var translate3d = 'translate3d(-' + dtop + 'px, 0px, 0px)';
		...
		/**
		 * Déplacement des slides avec les flèches directionnelles, verticalement et horizontalement
		 */
		...
		*/
		function landscapeScroll(slides, destiny){
			...
			if(options.css3){
				var translate3d = 'translate3d(0px, -' + destinyPos.left + 'px, 0px)';
